        ...
        if (
            ...
            for i in range(self.num_layers):
                for kv in ["key", "value"]:
                    ...
                    if kv == "key":
                        dynamic_axes[f"past_{kv}.{i}"] = pkv_dynamic_axes_key
                    else:
                        dynamic_axes[f"past_{kv}.{i}"] = pkv_dynamic_axes_value
            ...
            for i in range(self.num_layers):
                for kv in ["key", "value"]:
                    if kv == "key":
                        ...
                        dynamic_axes[f"past_{kv}.{i}"] = pkv_dynamic_axes_key[i]
                    else:
                        example_inputs["past_key_values"][i].append(torch.zeros(kv_cache_shape, dtype=torch.float32))
                        dynamic_axes[f"past_{kv}.{i}"] = pkv_dynamic_axes_value[i]
                    output_names.append(f"past_{kv}.{i}_RetainedState")
