...
class CtxScatterFuncKey(torch.autograd.Function):
    """
    ...
    def symbolic(g: torch.Graph, data: torch.Value, position_ids: torch.Value, updates: torch.Value) -> torch.Value:
        return g.onnxscript_op(CtxScatterKey, data, position_ids, updates).setTypeAs(data)


class CtxScatterFuncValue(torch.autograd.Function):
    """
    Function to scatter the current key values into KV-cache.
    """
...
def CtxGatherKey(
    ...
    shape_tensor = ops.Concat(ops.Shape(data)[:3], ops.Reshape(comp_ctx_len, [1]), axis=0)

    ctx_indices = ops.Expand(ctx_indices, shape_tensor)
    ctx_indices = ops.Unsqueeze(ctx_indices, [-1])
    return ops.GatherND(data, ctx_indices, batch_dims=3)
...
class CtxGatherFuncKey(torch.autograd.Function):
    ...
    def forward(data: torch.Tensor, ctx_indices: torch.Tensor, comp_ctx_len: int):
        # ctx_idx = torch.where(ctx_indices == torch.iinfo(torch.int32).max, 0, ctx_indices)
        # index = ctx_idx.expand(data.shape[0], data.shape[1], data.shape[2], -1)
        # return torch.gather(data, dim=3, index=index)
        batch_idx = torch.arange(data.shape[0]).view(-1, 1, 1, 1)
        head_idx = torch.arange(data.shape[1]).view(1, -1, 1, 1)
        d_idx = torch.arange(data.shape[2]).view(1, 1, -1, 1)
        ctx_idx = torch.where(ctx_indices == torch.iinfo(torch.int32).max, 0, ctx_indices)
        return data[batch_idx, head_idx, d_idx, ctx_idx]
    ...
    @staticmethod
    def symbolic(g: torch.Graph, data: torch.Value, ctx_indices: torch.Value, comp_ctx_len) -> torch.Value:
        return g.onnxscript_op(CtxGatherKey, data, ctx_indices, comp_ctx_len).setTypeAs(data)


class CtxGatherFuncValue(torch.autograd.Function):
    """
    Function to gather only the valid key values from KV-cache.
    """
